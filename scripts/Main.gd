...
func _ready():
	...
	faction_list_label.add_theme_font_size_override("font_size", 24)
...
func _init_factions():
	# 全てが同時に滅亡しないよう、初期兵力に少しランダムな揺らぎを入れる
...
func _update_simulation(delta):
	for faction in factions:
		if faction.is_alive():
			faction.total_power += 2000 * delta # 毎秒2000の回復
	...
	for pair in adjacency:
		...
		if c1.faction != c2.faction and c1.faction.is_alive() and c2.faction.is_alive():
			# 対称的になりすぎないように少しランダムなダメージ
			damage_to_deal[c1.faction] += randf_range(4000, 6000) * delta
			damage_to_deal[c2.faction] += randf_range(4000, 6000) * delta
